...
export function OttExperience({ videos, categories, mode = "home" }: { videos: Video[]; categories: VideoCategory[]; mode?: "home" | "explore" | "search" | "reels" }) {
  const [language, setLanguage] = useState<Language | null>(null); const [pickerOpen, setPickerOpen] = useState(false);
  ...
  const activeVideos = useMemo(() => language ? videos.filter((video) => video.language === language) : videos, [videos, language]);
  if (!language) return <main className="ott-page"><section className="ott-welcome"><span className="ott-cross">✦</span><p className="ott-kicker">ELROI OTT</p><h1>A place for faith, hope, and every heart.</h1><p>Begin in the language that feels like home.</p><LanguagePicker value={language} onChange={choose} /></section></main>;
  if (mode === "explore" || mode === "search") return <main className="ott-page ott-subpage"><LanguagePicker compact value={language} onChange={choose} /><SearchAndFilters videos={videos} categories={categories} initialSearch={mode === "search"} /></main>;
  if (mode === "reels") { const reels = activeVideos.filter((video) => video.isReel); return <main className="ott-page ott-reels-page"><LanguagePicker compact value={language} onChange={choose} /><header><span className="ott-kicker">ELROI REELS</span><h1>Small moments. Lasting hope.</h1></header><div className="ott-reels-feed">{reels.map((video) => <Reel key={video.id} video={video} />)}</div>{!reels.length && <div className="ott-no-results"><Play size={27} /><h2>Reels are coming soon.</h2><p>New moments of hope will appear here.</p></div>}</main>; }
  ...
  return <main className="ott-page"><Hero videos={hero} /><div className="ott-home-content"><div className="ott-language-row"><div><span className="ott-kicker">WATCH IN YOUR LANGUAGE</span><h2>Made for your faith journey.</h2></div><LanguagePicker compact value={language} onChange={choose} /></div><div className="ott-category-chips"><Link href="/ott/explore">All stories</Link>{categories.map((category) => <Link key={category.id} href={`/ott/explore?category=${category.slug}`}>{category.name}</Link>)}</div>{shelfTypes.map((shelf, index) => { const items = index === 0 ? featured : shelf.type ? activeVideos.filter((video) => video.contentType === shelf.type && !video.isReel) : activeVideos.filter((video) => !video.isReel); return items.length ? <section className="ott-shelf" key={shelf.title}><div><span className="ott-kicker">{index === 0 ? "CURATED FOR YOU" : "ELROI OTT"}</span><h2>{shelf.title}</h2></div><div className="ott-card-rail">{items.slice(0, 12).map((video) => <Card key={video.id} video={video} />)}</div></section> : null; })}</div>{pickerOpen && <div className="ott-language-modal"><button type="button" onClick={() => setPickerOpen(false)} aria-label="Close language picker"><X /></button><LanguagePicker value={language} onChange={choose} /></div>}</main>;
}
