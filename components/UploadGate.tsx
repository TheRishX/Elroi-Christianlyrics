...
export function UploadGate() {
  ...
  async function login(event: FormEvent) {
    ...
    if (response.ok) window.location.reload(); else { const payload = await response.json().catch(() => ({})); setError(payload.error || "Unable to sign in. Please try again."); setBusy(false); }
  }
