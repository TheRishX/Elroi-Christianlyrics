...
export function VideoLibrary({ videos, categories }: { videos: Video[]; categories: VideoCategory[] }) {
  ...
  return <main className="videos-page page">
    <header className="videos-header"><div><span className="eyebrow">ELROI TUNES / WATCH</span><h1>Stories that strengthen faith.</h1><p>Lyrics videos, testimonies, worship sessions, and Christian stories in one place.</p></div><label className="video-search"><Search size={18} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search videos…" aria-label="Search videos" /></label></header>
