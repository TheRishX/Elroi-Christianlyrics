...
export default async function ArtistPage({ params }: { params: Promise<{ artist: string }> }) {
  ...
  const artistSongs = songs.filter((song) => (song.artists || [song.artist]).some((artist) => artistSlug(artist) === slug) || artistSlug(song.worshipTeam || "") === slug);
  if (!artistSongs.length) notFound();
  const name = artistSongs.find((song) => (song.artists || [song.artist]).some((artist) => artistSlug(artist) === slug))?.artists?.find((artist) => artistSlug(artist) === slug) || artistSongs.find((song) => artistSlug(song.worshipTeam || "") === slug)?.worshipTeam || slug;
