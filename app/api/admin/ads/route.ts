...
export async function PUT(request:Request){if(!await authorized())return NextResponse.json({error:"Unauthorized"},{status:401});const body=await request.json().catch(()=>({}));try{const settings=(await wordpressSettings("PUT",body))||{...defaults,...body};revalidateTag("ads","max");return NextResponse.json({settings},{headers:{"Cache-Control":"no-store"}});}catch{return NextResponse.json({error:"Settings could not be saved."},{status:503});}}
